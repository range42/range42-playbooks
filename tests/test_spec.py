...
def _minimal(boxes: list[dict] | None = None) -> dict:
    return {
        "schema_version": 1,
        "name": "test_lab",
        "subnet_layout": "dual-lan",
        "boxes": boxes or [{"template": "debian-jump", "subnet": "lan1"}],
    }
...
# ---------------------------------------------------------------------------
# Auto-inject services.apt
# ---------------------------------------------------------------------------

def test_auto_inject_apt_cache_sets_proxy_mode():
    spec = ScenarioSpec.model_validate(_minimal([
        {"template": "apt-cache", "subnet": "lan1"},
        {"template": "debian-jump", "subnet": "lan2"},
    ]))
...
def test_auto_inject_apt_mirror_sets_mirror_mode():
    spec = ScenarioSpec.model_validate(_minimal([
        {"template": "apt-mirror", "subnet": "lan1"},
        {"template": "ubuntu-jump", "subnet": "lan2"},
    ]))
    assert spec.services.apt.box == "apt-mirror"
    assert spec.services.apt.mode == "mirror"


def test_auto_inject_apt_mirror_airgapped_sets_mirror_mode():
    spec = ScenarioSpec.model_validate(_minimal([
        {"template": "apt-mirror-airgapped", "subnet": "lan1"},
        {"template": "debian-jump", "subnet": "lan2"},
    ]))
    assert spec.services.apt.box == "apt-mirror-airgapped"
    assert spec.services.apt.mode == "mirror"


def test_no_auto_inject_without_apt_provider():
    spec = ScenarioSpec.model_validate(_minimal([
        {"template": "debian-jump", "subnet": "lan1"},
    ]))
    assert spec.services is None


def test_no_auto_inject_when_services_explicitly_disabled():
    data = _minimal([
        {"template": "apt-cache", "subnet": "lan1"},
        {"template": "debian-jump", "subnet": "lan2"},
    ])
    data["services"] = {"apt": None}
    spec = ScenarioSpec.model_validate(data)
    assert spec.services.apt is None


def test_no_auto_inject_when_services_explicitly_set():
    data = _minimal([
        {"template": "apt-cache", "subnet": "lan1"},
        {"template": "debian-jump", "subnet": "lan2"},
    ])
    data["services"] = {"apt": {"box": "apt-cache", "mode": "proxy", "wire_to": ["debian-jump"]}}
    spec = ScenarioSpec.model_validate(data)
    assert spec.services.apt.wire_to == ["debian-jump"]


def test_no_auto_inject_with_two_apt_providers():
    spec = ScenarioSpec.model_validate(_minimal([
        {"template": "apt-cache",  "subnet": "lan1"},
        {"template": "apt-mirror", "subnet": "lan1"},
        {"template": "debian-jump", "subnet": "lan2"},
