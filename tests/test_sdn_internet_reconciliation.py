...
@pytest.mark.parametrize(('mode', 'initial', 'desired'), [('on', 0, 1), ('off', 1, 0), ('toggle', 0, 1), ('toggle', 1, 0)])
def test_apply_reconciles_every_declared_subnet_without_changing_other_intent(tmp_path, mode, initial, desired):
    """A host-wide apply replays other subnets' hooks, so all need deduplication."""
...
''')
    ...
    fixture = [
        {'subnet': subnet_id, 'subnet_vnet': 'target', 'subnet_cidr': '10.80.1.0/24', 'subnet_snat': initial},
        {'subnet': 'zone-10.80.2.0-24', 'subnet_vnet': 'otheron', 'subnet_cidr': '10.80.2.0/24', 'subnet_snat': 1},
        {'subnet': 'zone-10.80.3.0-24', 'subnet_vnet': 'otheroff', 'subnet_cidr': '10.80.3.0/24', 'subnet_snat': 0},
        {'subnet': 'unknown-cidr', 'subnet_vnet': 'unknown'},
        {'subnet': 'empty-cidr', 'subnet_vnet': 'empty', 'subnet_cidr': ''},
    ]
    inventory = tmp_path / 'hosts.ini'
    inventory.write_text(f'proxmox ansible_connection=local ansible_python_interpreter={sys.executable}\n')
...
''')
    ...
    extra.write_text(json.dumps({'BUNDLE_SDN_SUBNET_ID': subnet_id, 'fixture_subnets': fixture}))
    result = subprocess.run([str(Path(sys.executable).with_name('ansible-playbook')), '-i', str(inventory), str(wrapper), '-e', f'@{extra}'],
                            env={**os.environ, 'RANGE42_ACTIVE_CONFIG_DIR': str(config), 'ANSIBLE_ROLES_PATH': str(tmp_path / 'roles'), 'ANSIBLE_NOCOLOR': '1'},
    ...
    assert observed['reconciled'] == [
        ...
        {'cidr': '10.80.2.0/24', 'want': 1},
        {'cidr': '10.80.3.0/24', 'want': 0},
    ]
