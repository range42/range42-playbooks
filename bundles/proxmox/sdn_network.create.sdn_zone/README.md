# sdn_network.create.sdn_zone

Create an SDN zone. One raw API call.
...
The zone is the container : a vnet cannot be created before its zone exists, and a zone cannot be deleted while it still holds one.
...
- the devkit equivalent, for a shell one-liner rather than a playbook:
  `proxmox_network.*.add_sdn_zone.to.jsons.sh`
