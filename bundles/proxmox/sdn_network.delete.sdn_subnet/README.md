# sdn_network.delete.sdn_subnet

Delete one subnet of a vnet. One raw API call.
...
Delete order is imposed by Proxmox : subnet, then vnet, then zone. And a subnet declared snat=1 leaves its iptables rule behind unless the post-down runs - see sdn_network.reconcile.snat_rules.
...
- the devkit equivalent, for a shell one-liner rather than a playbook:
  `proxmox_network.*.delete_sdn_subnet.to.jsons.sh`
