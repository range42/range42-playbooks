# sdn_network.list.sdn_zones

Read the SDN zones of the cluster. One action, no write, no apply.
...
No caller-passed parameter: `GET /cluster/sdn/zones` is cluster-level and returns
everything. Filtering is the consumer's job, on the resulting fact.

## Output

The role sets the fact **`network_list_sdn_zones`** - a list holding one dict per zone - and prints
it. Keys per entry:
...
`zone_type` is `simple` for every zone this project creates - host-local, no VLAN, no VXLAN.
...
against ansible-core). So `zone_state` or `zone_pending` may simply not be there, and any
...
`zone_pending` and `zone_state` are what tell you the running config has not caught up with the
...
- the devkit equivalent, for a shell one-liner rather than a playbook:
  `proxmox_network.datacenter.list_sdn_zones.to.jsons.sh`
