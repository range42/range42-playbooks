# sdn_network.create.sdn_subnet

Create a subnet on a vnet. One raw API call.
...
| `BUNDLE_SDN_SUBNET_CIDR` | yes | `sdn_subnet` | The subnet as a CIDR, e. |
...
The action reads the real id back from the API after writing, rather than trusting the derivation - so a divergence surfaces here instead of being assumed away.
...
- the devkit equivalent, for a shell one-liner rather than a playbook:
  `proxmox_network.*.add_sdn_subnet.to.jsons.sh`
