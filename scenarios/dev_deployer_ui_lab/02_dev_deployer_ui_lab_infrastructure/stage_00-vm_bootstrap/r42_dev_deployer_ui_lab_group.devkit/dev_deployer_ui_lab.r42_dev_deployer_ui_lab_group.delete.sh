...
##
## VM filter is derived from manifest/scenario_vms.json (role=dev-deployer-ui-lab),
...
mapfile -t VM_IDS < <(jq -r '.vms[] | select(.role == "dev-deployer-ui-lab") | .vm_id' "$MANIFEST")
mapfile -t VM_IPS < <(jq -r '.vms[] | select(.role == "dev-deployer-ui-lab") | .ip'    "$MANIFEST")
