...
##
## nextcloud_lab.delete_vms_only.sh - destroy the nextcloud_lab VM, keep templates
##
## VM IDs are read from the scenario manifest :
##   manifest/scenario_vms.json
##
## Lifted from catalog_try.delete_vms_only.sh, scoped to the nextcloud_lab manifest
## (currently lists only admin-nextcloud-standalone, VMID 1181, IP 192.168.142.181).
##
...
echo ":: stopping and deleting nextcloud_lab VMs (keeping templates)..."
echo ":: nextcloud_lab VMs: ${SCENARIO_VM_IDS[*]}"
echo ""
...
echo ":: redeploy with: range42-context deploy-vms  (or ./nextcloud_lab.setup_vms_only.sh)"
echo ""
