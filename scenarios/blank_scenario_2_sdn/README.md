...
## Two things to know meanwhile

Same vm_ids and IPs as `blank_scenario_2_subnets`, so **the two are mutually exclusive on one
hypervisor**. Deliberate.

**Subnet isolation is not part of this scenario** - the subnets reach each other today. Expected
state, not a defect.
