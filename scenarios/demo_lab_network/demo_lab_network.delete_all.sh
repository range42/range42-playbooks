...
##
## WARNING: Templates (9xxx) are deleted by this script. They may be shared with
##   other scenarios on the same Proxmox (demo_lab / blank_scenario_* all use the
##   same template IDs). Run this only when no other scenario relies on them, or
##   run delete_vms_only.sh to keep templates.
##
## NOTE: This script does NOT remove the iptables FORWARD rules applied to the
##   Proxmox host by 05_network_isolation. Clean those up manually if needed.
##
