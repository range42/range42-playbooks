...
| `03_trainer_infrastructure/stage_01-vm_configure/` | Docker baseline + dotfiles + firewall (port 22) + kunai-project repos clone on the trainer. Uses `systems.baseline.docker_host` + `network-baseline-ssh` + `tailscale-on-group` (gated off). |
