...
##
## VM filter is derived from manifest/scenario_vms.json (group=trainer),
...
mapfile -t VM_IDS < <(jq -r '.vms[] | select(.group == "trainer") | .vm_id' "$MANIFEST")
mapfile -t VM_IPS < <(jq -r '.vms[] | select(.group == "trainer") | .ip'    "$MANIFEST")
