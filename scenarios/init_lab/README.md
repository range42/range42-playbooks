...
**Formerly the bridge-based `_init_lab`** (its own template playbooks on `vmbr140`, five init VMs on `vmbr142`) : rebuilt on the SDN pilot's shape on 2026-09-21, same purpose, same init vm_ids 900 to 904, and renamed `init_lab` the same day : the init wizard treats a leading underscore as a placeholder marker and does not list such a directory. The templates now come from the shared bundles `template.build.ubuntu_noble` and `template.build.alpine`, exactly as every other scenario builds them.
...
| Script                                                          | Action                                                              |
|-----------------------------------------------------------------|---------------------------------------------------------------------|
| `init_lab.setup.sh`                     | Run main playbook (templates + VMs + optional admin)                |
| `init_lab.setup_vms_only.sh`            | Run main_vms_only.yml (skip template creation)                      |
| `init_lab.reset.setup.sh`               | Delete + redeploy VMs                                               |
| `init_lab.reset.ssh_keys.sh`            | Clear ~/.ssh/known_hosts for every IP in manifest                   |
| `init_lab.delete_vms_only.sh`           | Delete VMs only, keep templates                                     |
| `init_lab.delete_all.sh`                | Delete VMs + templates (WARNING - affects other scenarios)          |
| `init_lab.setup_networks.sh`            | Create the SDN zone, vnets and subnets declared in `00_sdn_bootstrap/_main.yml` (`--dry-run` compares without writing) |
| `init_lab.delete_networks.sh`           | Remove those subnets and vnets - the shared zone is kept            |
