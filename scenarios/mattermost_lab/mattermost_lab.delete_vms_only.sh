...
##
## mattermost_lab.delete_vms_only.sh - destroy the mattermost_lab VM, keep templates
##
## VM IDs are read from the scenario manifest :
##   manifest/scenario_vms.json
##
## Lifted from catalog_try.delete_vms_only.sh, scoped to the mattermost_lab manifest
## (currently lists only admin-mattermost-standalone, VMID 1182, IP 192.168.142.182).
##
...
echo ":: stopping and deleting mattermost_lab VMs (keeping templates)..."
echo ":: mattermost_lab VMs: ${SCENARIO_VM_IDS[*]}"
echo ""
...
echo ":: redeploy with: range42-context deploy-vms  (or ./mattermost_lab.setup_vms_only.sh)"
echo ""
