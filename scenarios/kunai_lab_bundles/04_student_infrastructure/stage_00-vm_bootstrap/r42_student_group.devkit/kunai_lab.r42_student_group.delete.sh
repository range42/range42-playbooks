...
##
## VM filter is derived from manifest/scenario_vms.json (group=student),
...
mapfile -t VM_IDS < <(jq -r '.vms[] | select(.group == "student") | .vm_id' "$MANIFEST")
mapfile -t VM_IPS < <(jq -r '.vms[] | select(.group == "student") | .ip'    "$MANIFEST")
